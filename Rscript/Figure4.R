# =============================================================================
#   Figure 4 - pan-disease application (12 diseases, GMrepo)
#
#  Run from the project root: Rscript Rscript/Figure4.R
# =============================================================================
...
# =============================================================================
  ...
  rename_map <- c(
    ...
    "Obesity"                           = "Obesity",
    "Psoriasis"                         = "Psoriasis",
    "Scleroderma, Systemic"             = "Systemic sclerosis",
...
# =============================================================================
  ...
  n_studies  <- length(summary_stat_meta_filter)

  ## Per-study estimates, standard errors and FDR q-values, aligned on
  ## feature.ID.  The saved summary statistics cover a wider feature set than
  ## the model, so this also restricts them to the modelled features.
  ...
  ## ---- display order ---------------------------------------------------------
  ## Clusters: fewest selected features first.
  ...
  ggsave(
    ...
    plot = g_A1,
    width = 800, height = 140, units = "mm", dpi = 300
  )
  ...
  ggsave(
    ...
    plot = g_A2,
    width = 800, height = 170, units = "mm", dpi = 300
  )
...
# =============================================================================
  ...
  ## "C2-specific (+)", ...) is derived from `beta` by signature_label() rather
  ## than hard-coded, so the subtitles stay correct if the fit or the cluster
  ## order changes.
  ##
  ...
  panel_c_taxa <- c(
    ...
    "Blautia hansenii [1322]",
    "Mediterraneibacter gnavus [33038]",
    "Eggerthella lenta [84112]",
    "Escherichia coli [562]"
  )

  ## Feature IDs carry an NCBI taxon id; drop it for the italic subtitle.
  pretty_taxon <- function(x) sub(" \\[[0-9]+\\]$", "", x)
  ...
  g_C_lst <- lapply(panel_c_taxa, function(taxon) {
    ...
    ggplot(df_forest, aes(y = id, x = est, color = cluster)) +
      ...
      geom_errorbar(aes(xmin = lower, xmax = upper),
                    orientation = "y", width = 0, linewidth = 1.2) +
      geom_point(size = 2) +
      ## direction = "none": the sign is already visible from the intervals.
      ggtitle(bquote(atop(bold(.(signature_label(taxon, beta, cluster_order,
                                                 direction = "none"))),
...
# =============================================================================
  ...
  ggsave(
    ...
    plot = g_D,
    width = 135, height = 60, units = "mm", dpi = 300
  )
...
# =============================================================================
  ...
  ggsave(
    ...
    plot = g_E,
    width = 380, height = 90, units = "mm", dpi = 300
  )
